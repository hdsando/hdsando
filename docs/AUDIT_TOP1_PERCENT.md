...
| B6 | Journal d'audit : la vérification recalculait avec le provider courant | Journal écrit sous .NET non vérifiable sans .NET | **Corrigé** : algorithme enregistré par ligne (col. H), `Crypto_Provider.HashHexWith` ; lignes non vérifiables comptées, pas déclarées corrompues |
| B9 | **Deux écrivains** pour `AUDIT_TRAIL` : `Core_Engine.WriteToAuditLog` (6 col., hash non chaîné sur le n° de ligne) et `SAFA_Common.WriteAuditLog` (7 col., chaîné) ; horodatage hashé ≠ horodatage écrit ; textes tronqués après hachage | La vérification d'intégrité **échouait toujours** et affichait « intégrité compromise » | **Corrigé** : un seul écrivain, valeurs hashées = valeurs écrites, vérification complète (chaîne **et** hash) silencieuse avec rapport (`LastIntegrityReport`) |
